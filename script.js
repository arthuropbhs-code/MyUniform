const RIBBONS=[{id:'hero',name:'Medal for Heroism',precedence:1,img:'images/hero.png'}, /* add rest */];
...
function updatePreview(){
    ...
    for(let i=0;i<activeRibbons.length;i+=3){const row=document.createElement('div');row.className='flex gap-[1px]';activeRibbons.slice(i,i+3).forEach(r=>{const count=state.ribbons.get(r.id);const div=document.createElement('div');div.className='ribbon-mini';div.style.backgroundImage=`url(${r.img})`;if(count>1){const star=document.createElement('div');star.className='mini-star';star.innerText=count>5?'★S':'★'.repeat(count-1);div.appendChild(star);}row.appendChild(div);});rPreview.appendChild(row);}
